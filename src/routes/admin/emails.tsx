...
import { renderToString, Show } from 'solid-js/web'
import { TestEmail } from '~/emails/TestEmail'

const imports = import.meta.glob(`../../emails/TestEmail.tsx`, { import: 'default', query: '?raw' })
const source = (await Promise.all(Object.values(imports).map((imp) => imp()))).at(0)

const getEmailHtml = query(async () => {
	'use server'
	return { html: renderToString(() => <TestEmail />), source }
}, 'test-email')

export default function EmailsPage() {
	const emailData = createAsync(() => getEmailHtml())
	return (
		<div class="tabs tabs-border h-full" role="tablist">
			...
			<div class="tab-content bg-neutral text-neutral-content border-base-300 p-6" role="tabpanel">
				<pre class="[tab-size:2]">{emailData()?.source as string}</pre>
			</div>
		</div>
	)
}
